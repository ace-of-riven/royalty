...
*/ 
void ChessBoard::printMove (Piece* movingPiece, string sourceFileRank, string destFileRank) {
...
Piece *ChessBoard::at ( char i , char j ) const {
	string fileRank ( { j, i } );
	Piece *piece = NULL;
	try {
		piece = board->at ( fileRank );
	}
	catch ( const std::out_of_range &err ) {
		return NULL;
	}
...
Piece *ChessBoard::at ( std::string fileRank ) const {
	Piece *piece = NULL;
	try {
		piece = board->at ( fileRank );
	}
