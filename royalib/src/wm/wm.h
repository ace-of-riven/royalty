...
public:
	...
	int winmain_run(HINSTANCE hInst, int cmdShow) {
		...
		if (!UpdateWindow(this->hwnd())) {
			throw std::system_error(GetLastError(), std::system_category(), "UpdateWindow failed");
		}
...
private:
	void _init_setup_styles() noexcept {
		this->setup.wndClassEx.hbrBackground = reinterpret_cast<HBRUSH>(COLOR_BACKGROUND);
		this->setup.wndClassEx.hCursor = LoadCursorW(nullptr, reinterpret_cast<LPCWSTR>(IDC_ARROW));
		this->setup.wndClassEx.style = CS_HREDRAW | CS_VREDRAW | CS_OWNDC | CS_DBLCLKS;
		...
		this->setup.wndClassEx.lpszClassName = L"Royalty";
