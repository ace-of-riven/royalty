...
Batch::Batch ( GPUPrimType prim , GPU_Shader *shader ) : shader ( shader ) , prim ( prim ) {
...
void Batch::InsertMesh ( const Transform &transform , const Mesh *mesh ) {
	...
	for ( unsigned int v = 0; v < mesh->VerticesLen ( ); v++ ) {
		if ( aPos + 1 )
			GPU_vertbuf_attr_set ( vbo , aPos , vertices_count , glm::value_ptr ( mesh->Vertices ( ) [ v ].coord ) );
		if ( aUV + 1 )
			GPU_vertbuf_attr_set ( vbo , aUV , vertices_count , glm::value_ptr ( mesh->Vertices ( ) [ v ].uv ) );
		if ( aNorm + 1 )
			GPU_vertbuf_attr_set ( vbo , aNorm , vertices_count , glm::value_ptr ( mesh->Vertices ( ) [ v ].norm ) );
		if ( aMeshID + 1 )
			GPU_vertbuf_attr_set ( vbo , aMeshID , vertices_count , &mesh_count );
		vertices_count++;
	}

	for ( unsigned int i = 0; i < mesh->IndicesLen ( ); i++ ) {
		GPU_indexbuf_add_generic_vert ( builder , i_offset + mesh->Indices ( ) [ i ] );
		indices_count++;
	}
...
void Batch::Upload ( GPU_UniformBuf *mesh_ubo , GPU_UniformBuf *mat_ubo ) {
...
void Batch::Render ( GPU_UniformBuf *mesh_ubo , GPU_UniformBuf *mat_ubo ) const {
	if ( indices_count == 0 or vertices_count == 0 or mesh_count == 0 )
		return;
...
void Batch::UpdateMatrix ( const glm::mat4 &matrix ) {
	transform = matrix;
}

int Batch::RegisterTexture ( GPU_Texture *in ) {
...
int Batch::RegisterMaterial ( const Material *material ) {
...
bool Batch::HasSpaceForTexture ( const GPU_Texture *in ) {
...
bool Batch::HasSpaceForMaterial ( const Material *material ) {
	int material_id = materials.size ( );
	for ( unsigned int i = 0; i < material_id; i++ )
...
bool Batch::HasSpaceForMesh ( const Mesh *mesh ) {
	if ( vertices_count + mesh->VerticesLen ( ) >= MAX_VERTICES or
	     indices_count + mesh->IndicesLen ( ) >= MAX_INDICES or
