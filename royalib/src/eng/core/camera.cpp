...
unsigned int Camera::ComponentStack ( int m_type ) {
	switch ( m_type ) {
...
void Camera::OnUpdate ( double deltaTime ) {
	glm::vec3 m_pos = transform.Translation;
	...
	glm::mat4 vrot_mat = glm::rotate ( glm::rotate ( glm::mat4 ( 1 ) , m_rot.x , { 1, 0, 0 } ) , m_rot.y , { 0, 1, 0 } );
